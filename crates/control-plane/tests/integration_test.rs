...
///   - Wait for inter-group link targeting one of domain-b's nodes.
...
async fn test_reconnect_different_endpoint() {
    ...
    let link =
        wait_for_link_between_groups_entry(&mut client, "domain-a", "domain-b", SHORT_TIMEOUT).await;
