...
//! and `wasm_bindgen_futures::spawn_local` so a `wasm32-unknown-unknown` host can
//! drive a websocket connection from inside the browser event loop.
...
pub struct WebSocketStreams {
    pub inbound: ReceiverStream<Result<Message, Status>>,
    pub outbound: mpsc::Sender<Message>,
}

/// Spawn read/write loops over a browser `WebSocket`. The returned channels
/// look identical to the native flavor in `stream.rs` (an inbound
/// `Stream<Item = Result<Message, Status>>` plus an outbound `Sender<Message>`),
/// so the same connection/processing machinery can consume them without caring
/// which transport produced them.
pub fn spawn_transport_tasks(
