    ...
    async fn on_message(&mut self, message: SessionMessage) -> Result<SessionOutput, SessionError> {
        ...
        match message {
            SessionMessage::OnMessage {
                ...
                // if the participant is off-line it should refuse all the messages, except the RejoinRequest
                // that is generated by the resume function call done by the application. It also needs to
                // accept the RejoinReply message that is generated by the moderator when the participant is rejoining the session
                if self.common.participant_state == ParticipantState::OffLine
                    ...
                    && message.get_session_message_type() != ProtoSessionMessageType::RejoinReply
                {
                    debug!(
    ...
    async fn process_control_message(
        ...
        match message.get_session_message_type() {
            ...
            ProtoSessionMessageType::GroupAck => {
                ...
                // check if the task is still pending and if the message id match the pending task
                // the pending message in this case must be an UpdateParticipantState (the participant
                // is going offline)
                if !self.common.sender.is_still_pending(id)
                    && self.common.pending_status_update.is_some()
                {
