...
cfg_if::cfg_if! {
    if #[cfg(target_arch = "wasm32")] {
        mod wasm {
            ...
            /// Minimal browser-build configuration errors. The native `ConfigError`
            /// covers gRPC/TLS/auth/native-WebSocket; the wasm build only needs the
            /// surface exercised by `ClientConfig` validation and the gloo-net connect.
            #[derive(Error, Debug)]
