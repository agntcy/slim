// Copyright AGNTCY Contributors (https://github.com/agntcy)
// SPDX-License-Identifier: Apache-2.0

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        v5.29.0
// source: controlplane/v1/controlplane.proto

package controlplanev1

import (
	v1 "github.com/agntcy/slim/control-plane/common/proto/controller/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type NodeStatus int32

const (
	NodeStatus_NODE_STATUS_UNSPECIFIED NodeStatus = 0
	NodeStatus_CONNECTED               NodeStatus = 1
	NodeStatus_NOT_CONNECTED           NodeStatus = 2
)

// Enum value maps for NodeStatus.
var (
	NodeStatus_name = map[int32]string{
		0: "NODE_STATUS_UNSPECIFIED",
		1: "CONNECTED",
		2: "NOT_CONNECTED",
	}
	NodeStatus_value = map[string]int32{
		"NODE_STATUS_UNSPECIFIED": 0,
		"CONNECTED":               1,
		"NOT_CONNECTED":           2,
	}
)

func (x NodeStatus) Enum() *NodeStatus {
	p := new(NodeStatus)
	*p = x
	return p
}

func (x NodeStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (NodeStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_controlplane_v1_controlplane_proto_enumTypes[0].Descriptor()
}

func (NodeStatus) Type() protoreflect.EnumType {
	return &file_controlplane_v1_controlplane_proto_enumTypes[0]
}

func (x NodeStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use NodeStatus.Descriptor instead.
func (NodeStatus) EnumDescriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{0}
}

// Route status enum
type RouteStatus int32

const (
	RouteStatus_ROUTE_STATUS_UNSPECIFIED RouteStatus = 0
	RouteStatus_ROUTE_STATUS_APPLIED     RouteStatus = 1
	RouteStatus_ROUTE_STATUS_FAILED      RouteStatus = 2
)

// Enum value maps for RouteStatus.
var (
	RouteStatus_name = map[int32]string{
		0: "ROUTE_STATUS_UNSPECIFIED",
		1: "ROUTE_STATUS_APPLIED",
		2: "ROUTE_STATUS_FAILED",
	}
	RouteStatus_value = map[string]int32{
		"ROUTE_STATUS_UNSPECIFIED": 0,
		"ROUTE_STATUS_APPLIED":     1,
		"ROUTE_STATUS_FAILED":      2,
	}
)

func (x RouteStatus) Enum() *RouteStatus {
	p := new(RouteStatus)
	*p = x
	return p
}

func (x RouteStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RouteStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_controlplane_v1_controlplane_proto_enumTypes[1].Descriptor()
}

func (RouteStatus) Type() protoreflect.EnumType {
	return &file_controlplane_v1_controlplane_proto_enumTypes[1]
}

func (x RouteStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RouteStatus.Descriptor instead.
func (RouteStatus) EnumDescriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{1}
}

type ConfigurationCommand struct {
	state                protoimpl.MessageState   `protogen:"open.v1"`
	ConfigurationCommand *v1.ConfigurationCommand `protobuf:"bytes,1,opt,name=configuration_command,json=configurationCommand,proto3" json:"configuration_command,omitempty"`
	NodeId               string                   `protobuf:"bytes,2,opt,name=nodeId,proto3" json:"nodeId,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *ConfigurationCommand) Reset() {
	*x = ConfigurationCommand{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigurationCommand) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigurationCommand) ProtoMessage() {}

func (x *ConfigurationCommand) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigurationCommand.ProtoReflect.Descriptor instead.
func (*ConfigurationCommand) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{0}
}

func (x *ConfigurationCommand) GetConfigurationCommand() *v1.ConfigurationCommand {
	if x != nil {
		return x.ConfigurationCommand
	}
	return nil
}

func (x *ConfigurationCommand) GetNodeId() string {
	if x != nil {
		return x.NodeId
	}
	return ""
}

type Node struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Node) Reset() {
	*x = Node{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Node) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Node) ProtoMessage() {}

func (x *Node) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Node.ProtoReflect.Descriptor instead.
func (*Node) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{1}
}

func (x *Node) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type NodeListRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeListRequest) Reset() {
	*x = NodeListRequest{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeListRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeListRequest) ProtoMessage() {}

func (x *NodeListRequest) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeListRequest.ProtoReflect.Descriptor instead.
func (*NodeListRequest) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{2}
}

type NodeEntry struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Id            string                  `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Connections   []*v1.ConnectionDetails `protobuf:"bytes,2,rep,name=connections,proto3" json:"connections,omitempty"`
	Status        NodeStatus              `protobuf:"varint,4,opt,name=status,proto3,enum=controlplane.proto.v1.NodeStatus" json:"status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeEntry) Reset() {
	*x = NodeEntry{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeEntry) ProtoMessage() {}

func (x *NodeEntry) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeEntry.ProtoReflect.Descriptor instead.
func (*NodeEntry) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{3}
}

func (x *NodeEntry) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *NodeEntry) GetConnections() []*v1.ConnectionDetails {
	if x != nil {
		return x.Connections
	}
	return nil
}

func (x *NodeEntry) GetStatus() NodeStatus {
	if x != nil {
		return x.Status
	}
	return NodeStatus_NODE_STATUS_UNSPECIFIED
}

type NodeListResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Entries       []*NodeEntry           `protobuf:"bytes,1,rep,name=entries,proto3" json:"entries,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NodeListResponse) Reset() {
	*x = NodeListResponse{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NodeListResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NodeListResponse) ProtoMessage() {}

func (x *NodeListResponse) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NodeListResponse.ProtoReflect.Descriptor instead.
func (*NodeListResponse) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{4}
}

func (x *NodeListResponse) GetEntries() []*NodeEntry {
	if x != nil {
		return x.Entries
	}
	return nil
}

type AddRouteRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Connection    *v1.Connection         `protobuf:"bytes,1,opt,name=connection,proto3" json:"connection,omitempty"`
	Subscription  *v1.Subscription       `protobuf:"bytes,2,opt,name=subscription,proto3" json:"subscription,omitempty"`
	NodeId        string                 `protobuf:"bytes,3,opt,name=nodeId,proto3" json:"nodeId,omitempty"`
	DestNodeId    string                 `protobuf:"bytes,4,opt,name=destNodeId,proto3" json:"destNodeId,omitempty"` // optional
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddRouteRequest) Reset() {
	*x = AddRouteRequest{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddRouteRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddRouteRequest) ProtoMessage() {}

func (x *AddRouteRequest) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddRouteRequest.ProtoReflect.Descriptor instead.
func (*AddRouteRequest) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{5}
}

func (x *AddRouteRequest) GetConnection() *v1.Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

func (x *AddRouteRequest) GetSubscription() *v1.Subscription {
	if x != nil {
		return x.Subscription
	}
	return nil
}

func (x *AddRouteRequest) GetNodeId() string {
	if x != nil {
		return x.NodeId
	}
	return ""
}

func (x *AddRouteRequest) GetDestNodeId() string {
	if x != nil {
		return x.DestNodeId
	}
	return ""
}

type AddRouteResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	RouteId       string                 `protobuf:"bytes,2,opt,name=routeId,proto3" json:"routeId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddRouteResponse) Reset() {
	*x = AddRouteResponse{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddRouteResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddRouteResponse) ProtoMessage() {}

func (x *AddRouteResponse) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddRouteResponse.ProtoReflect.Descriptor instead.
func (*AddRouteResponse) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{6}
}

func (x *AddRouteResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *AddRouteResponse) GetRouteId() string {
	if x != nil {
		return x.RouteId
	}
	return ""
}

type DeleteRouteRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Subscription  *v1.Subscription       `protobuf:"bytes,1,opt,name=subscription,proto3" json:"subscription,omitempty"`
	NodeId        string                 `protobuf:"bytes,2,opt,name=nodeId,proto3" json:"nodeId,omitempty"`
	DestNodeId    string                 `protobuf:"bytes,3,opt,name=destNodeId,proto3" json:"destNodeId,omitempty"` // optional
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRouteRequest) Reset() {
	*x = DeleteRouteRequest{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRouteRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRouteRequest) ProtoMessage() {}

func (x *DeleteRouteRequest) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRouteRequest.ProtoReflect.Descriptor instead.
func (*DeleteRouteRequest) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{7}
}

func (x *DeleteRouteRequest) GetSubscription() *v1.Subscription {
	if x != nil {
		return x.Subscription
	}
	return nil
}

func (x *DeleteRouteRequest) GetNodeId() string {
	if x != nil {
		return x.NodeId
	}
	return ""
}

func (x *DeleteRouteRequest) GetDestNodeId() string {
	if x != nil {
		return x.DestNodeId
	}
	return ""
}

type DeleteRouteResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRouteResponse) Reset() {
	*x = DeleteRouteResponse{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRouteResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRouteResponse) ProtoMessage() {}

func (x *DeleteRouteResponse) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRouteResponse.ProtoReflect.Descriptor instead.
func (*DeleteRouteResponse) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{8}
}

func (x *DeleteRouteResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

type CreateChannelRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// list of moderators for the channel
	Moderators    []string `protobuf:"bytes,1,rep,name=moderators,proto3" json:"moderators,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateChannelRequest) Reset() {
	*x = CreateChannelRequest{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateChannelRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateChannelRequest) ProtoMessage() {}

func (x *CreateChannelRequest) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateChannelRequest.ProtoReflect.Descriptor instead.
func (*CreateChannelRequest) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{9}
}

func (x *CreateChannelRequest) GetModerators() []string {
	if x != nil {
		return x.Moderators
	}
	return nil
}

type CreateChannelResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the channel, to which clients will subscribe
	ChannelName   string `protobuf:"bytes,1,opt,name=channel_name,json=channelName,proto3" json:"channel_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateChannelResponse) Reset() {
	*x = CreateChannelResponse{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateChannelResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateChannelResponse) ProtoMessage() {}

func (x *CreateChannelResponse) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateChannelResponse.ProtoReflect.Descriptor instead.
func (*CreateChannelResponse) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{10}
}

func (x *CreateChannelResponse) GetChannelName() string {
	if x != nil {
		return x.ChannelName
	}
	return ""
}

// Request to list routes, filtered by optional nodeId and destNodeId
type RouteListRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// the identifier of the source node to filter routes
	SrcNodeId     string `protobuf:"bytes,1,opt,name=srcNodeId,proto3" json:"srcNodeId,omitempty"`   // optional
	DestNodeId    string `protobuf:"bytes,2,opt,name=destNodeId,proto3" json:"destNodeId,omitempty"` // optional
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RouteListRequest) Reset() {
	*x = RouteListRequest{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RouteListRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RouteListRequest) ProtoMessage() {}

func (x *RouteListRequest) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RouteListRequest.ProtoReflect.Descriptor instead.
func (*RouteListRequest) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{11}
}

func (x *RouteListRequest) GetSrcNodeId() string {
	if x != nil {
		return x.SrcNodeId
	}
	return ""
}

func (x *RouteListRequest) GetDestNodeId() string {
	if x != nil {
		return x.DestNodeId
	}
	return ""
}

// Route message representing a single route
type RouteEntry struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Id             uint64                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	SourceNodeId   string                 `protobuf:"bytes,2,opt,name=source_node_id,json=sourceNodeId,proto3" json:"source_node_id,omitempty"`
	DestNodeId     string                 `protobuf:"bytes,3,opt,name=dest_node_id,json=destNodeId,proto3" json:"dest_node_id,omitempty"`
	DestEndpoint   string                 `protobuf:"bytes,4,opt,name=dest_endpoint,json=destEndpoint,proto3" json:"dest_endpoint,omitempty"`
	ConnConfigData string                 `protobuf:"bytes,5,opt,name=conn_config_data,json=connConfigData,proto3" json:"conn_config_data,omitempty"`
	Component_0    string                 `protobuf:"bytes,6,opt,name=component_0,json=component0,proto3" json:"component_0,omitempty"`
	Component_1    string                 `protobuf:"bytes,7,opt,name=component_1,json=component1,proto3" json:"component_1,omitempty"`
	Component_2    string                 `protobuf:"bytes,8,opt,name=component_2,json=component2,proto3" json:"component_2,omitempty"`
	ComponentId    *uint64                `protobuf:"varint,9,opt,name=component_id,json=componentId,proto3,oneof" json:"component_id,omitempty"`
	Status         RouteStatus            `protobuf:"varint,10,opt,name=status,proto3,enum=controlplane.proto.v1.RouteStatus" json:"status,omitempty"`
	StatusMsg      string                 `protobuf:"bytes,11,opt,name=status_msg,json=statusMsg,proto3" json:"status_msg,omitempty"`
	Deleted        bool                   `protobuf:"varint,12,opt,name=deleted,proto3" json:"deleted,omitempty"`
	LastUpdated    int64                  `protobuf:"varint,13,opt,name=last_updated,json=lastUpdated,proto3" json:"last_updated,omitempty"` // Unix timestamp
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RouteEntry) Reset() {
	*x = RouteEntry{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RouteEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RouteEntry) ProtoMessage() {}

func (x *RouteEntry) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RouteEntry.ProtoReflect.Descriptor instead.
func (*RouteEntry) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{12}
}

func (x *RouteEntry) GetId() uint64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *RouteEntry) GetSourceNodeId() string {
	if x != nil {
		return x.SourceNodeId
	}
	return ""
}

func (x *RouteEntry) GetDestNodeId() string {
	if x != nil {
		return x.DestNodeId
	}
	return ""
}

func (x *RouteEntry) GetDestEndpoint() string {
	if x != nil {
		return x.DestEndpoint
	}
	return ""
}

func (x *RouteEntry) GetConnConfigData() string {
	if x != nil {
		return x.ConnConfigData
	}
	return ""
}

func (x *RouteEntry) GetComponent_0() string {
	if x != nil {
		return x.Component_0
	}
	return ""
}

func (x *RouteEntry) GetComponent_1() string {
	if x != nil {
		return x.Component_1
	}
	return ""
}

func (x *RouteEntry) GetComponent_2() string {
	if x != nil {
		return x.Component_2
	}
	return ""
}

func (x *RouteEntry) GetComponentId() uint64 {
	if x != nil && x.ComponentId != nil {
		return *x.ComponentId
	}
	return 0
}

func (x *RouteEntry) GetStatus() RouteStatus {
	if x != nil {
		return x.Status
	}
	return RouteStatus_ROUTE_STATUS_UNSPECIFIED
}

func (x *RouteEntry) GetStatusMsg() string {
	if x != nil {
		return x.StatusMsg
	}
	return ""
}

func (x *RouteEntry) GetDeleted() bool {
	if x != nil {
		return x.Deleted
	}
	return false
}

func (x *RouteEntry) GetLastUpdated() int64 {
	if x != nil {
		return x.LastUpdated
	}
	return 0
}

// Response containing a list of route representations
type RouteListResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// list of routes
	Routes        []*RouteEntry `protobuf:"bytes,1,rep,name=routes,proto3" json:"routes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RouteListResponse) Reset() {
	*x = RouteListResponse{}
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RouteListResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RouteListResponse) ProtoMessage() {}

func (x *RouteListResponse) ProtoReflect() protoreflect.Message {
	mi := &file_controlplane_v1_controlplane_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RouteListResponse.ProtoReflect.Descriptor instead.
func (*RouteListResponse) Descriptor() ([]byte, []int) {
	return file_controlplane_v1_controlplane_proto_rawDescGZIP(), []int{13}
}

func (x *RouteListResponse) GetRoutes() []*RouteEntry {
	if x != nil {
		return x.Routes
	}
	return nil
}

var File_controlplane_v1_controlplane_proto protoreflect.FileDescriptor

const file_controlplane_v1_controlplane_proto_rawDesc = "" +
	"\n" +
	"\"controlplane/v1/controlplane.proto\x12\x15controlplane.proto.v1\x1a\x1econtroller/v1/controller.proto\"\x8e\x01\n" +
	"\x14ConfigurationCommand\x12^\n" +
	"\x15configuration_command\x18\x01 \x01(\v2).controller.proto.v1.ConfigurationCommandR\x14configurationCommand\x12\x16\n" +
	"\x06nodeId\x18\x02 \x01(\tR\x06nodeId\"\x16\n" +
	"\x04Node\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\"\x11\n" +
	"\x0fNodeListRequest\"\xa0\x01\n" +
	"\tNodeEntry\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12H\n" +
	"\vconnections\x18\x02 \x03(\v2&.controller.proto.v1.ConnectionDetailsR\vconnections\x129\n" +
	"\x06status\x18\x04 \x01(\x0e2!.controlplane.proto.v1.NodeStatusR\x06status\"N\n" +
	"\x10NodeListResponse\x12:\n" +
	"\aentries\x18\x01 \x03(\v2 .controlplane.proto.v1.NodeEntryR\aentries\"\xd1\x01\n" +
	"\x0fAddRouteRequest\x12?\n" +
	"\n" +
	"connection\x18\x01 \x01(\v2\x1f.controller.proto.v1.ConnectionR\n" +
	"connection\x12E\n" +
	"\fsubscription\x18\x02 \x01(\v2!.controller.proto.v1.SubscriptionR\fsubscription\x12\x16\n" +
	"\x06nodeId\x18\x03 \x01(\tR\x06nodeId\x12\x1e\n" +
	"\n" +
	"destNodeId\x18\x04 \x01(\tR\n" +
	"destNodeId\"F\n" +
	"\x10AddRouteResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\x12\x18\n" +
	"\arouteId\x18\x02 \x01(\tR\arouteId\"\x93\x01\n" +
	"\x12DeleteRouteRequest\x12E\n" +
	"\fsubscription\x18\x01 \x01(\v2!.controller.proto.v1.SubscriptionR\fsubscription\x12\x16\n" +
	"\x06nodeId\x18\x02 \x01(\tR\x06nodeId\x12\x1e\n" +
	"\n" +
	"destNodeId\x18\x03 \x01(\tR\n" +
	"destNodeId\"/\n" +
	"\x13DeleteRouteResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\"6\n" +
	"\x14CreateChannelRequest\x12\x1e\n" +
	"\n" +
	"moderators\x18\x01 \x03(\tR\n" +
	"moderators\":\n" +
	"\x15CreateChannelResponse\x12!\n" +
	"\fchannel_name\x18\x01 \x01(\tR\vchannelName\"P\n" +
	"\x10RouteListRequest\x12\x1c\n" +
	"\tsrcNodeId\x18\x01 \x01(\tR\tsrcNodeId\x12\x1e\n" +
	"\n" +
	"destNodeId\x18\x02 \x01(\tR\n" +
	"destNodeId\"\xe7\x03\n" +
	"\n" +
	"RouteEntry\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x04R\x02id\x12$\n" +
	"\x0esource_node_id\x18\x02 \x01(\tR\fsourceNodeId\x12 \n" +
	"\fdest_node_id\x18\x03 \x01(\tR\n" +
	"destNodeId\x12#\n" +
	"\rdest_endpoint\x18\x04 \x01(\tR\fdestEndpoint\x12(\n" +
	"\x10conn_config_data\x18\x05 \x01(\tR\x0econnConfigData\x12\x1f\n" +
	"\vcomponent_0\x18\x06 \x01(\tR\n" +
	"component0\x12\x1f\n" +
	"\vcomponent_1\x18\a \x01(\tR\n" +
	"component1\x12\x1f\n" +
	"\vcomponent_2\x18\b \x01(\tR\n" +
	"component2\x12&\n" +
	"\fcomponent_id\x18\t \x01(\x04H\x00R\vcomponentId\x88\x01\x01\x12:\n" +
	"\x06status\x18\n" +
	" \x01(\x0e2\".controlplane.proto.v1.RouteStatusR\x06status\x12\x1d\n" +
	"\n" +
	"status_msg\x18\v \x01(\tR\tstatusMsg\x12\x18\n" +
	"\adeleted\x18\f \x01(\bR\adeleted\x12!\n" +
	"\flast_updated\x18\r \x01(\x03R\vlastUpdatedB\x0f\n" +
	"\r_component_id\"N\n" +
	"\x11RouteListResponse\x129\n" +
	"\x06routes\x18\x01 \x03(\v2!.controlplane.proto.v1.RouteEntryR\x06routes*K\n" +
	"\n" +
	"NodeStatus\x12\x1b\n" +
	"\x17NODE_STATUS_UNSPECIFIED\x10\x00\x12\r\n" +
	"\tCONNECTED\x10\x01\x12\x11\n" +
	"\rNOT_CONNECTED\x10\x02*^\n" +
	"\vRouteStatus\x12\x1c\n" +
	"\x18ROUTE_STATUS_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14ROUTE_STATUS_APPLIED\x10\x01\x12\x17\n" +
	"\x13ROUTE_STATUS_FAILED\x10\x022\xbb\t\n" +
	"\x13ControlPlaneService\x12]\n" +
	"\bAddRoute\x12&.controlplane.proto.v1.AddRouteRequest\x1a'.controlplane.proto.v1.AddRouteResponse\"\x00\x12f\n" +
	"\vDeleteRoute\x12).controlplane.proto.v1.DeleteRouteRequest\x1a*.controlplane.proto.v1.DeleteRouteResponse\"\x00\x12a\n" +
	"\x11ListSubscriptions\x12\x1b.controlplane.proto.v1.Node\x1a-.controller.proto.v1.SubscriptionListResponse\"\x00\x12]\n" +
	"\x0fListConnections\x12\x1b.controlplane.proto.v1.Node\x1a+.controller.proto.v1.ConnectionListResponse\"\x00\x12^\n" +
	"\tListNodes\x12&.controlplane.proto.v1.NodeListRequest\x1a'.controlplane.proto.v1.NodeListResponse\"\x00\x12l\n" +
	"\rCreateChannel\x12+.controlplane.proto.v1.CreateChannelRequest\x1a,.controlplane.proto.v1.CreateChannelResponse\"\x00\x12V\n" +
	"\rDeleteChannel\x12).controller.proto.v1.DeleteChannelRequest\x1a\x18.controller.proto.v1.Ack\"\x00\x12X\n" +
	"\x0eAddParticipant\x12*.controller.proto.v1.AddParticipantRequest\x1a\x18.controller.proto.v1.Ack\"\x00\x12^\n" +
	"\x11DeleteParticipant\x12-.controller.proto.v1.DeleteParticipantRequest\x1a\x18.controller.proto.v1.Ack\"\x00\x12e\n" +
	"\fListChannels\x12(.controller.proto.v1.ListChannelsRequest\x1a).controller.proto.v1.ListChannelsResponse\"\x00\x12q\n" +
	"\x10ListParticipants\x12,.controller.proto.v1.ListParticipantsRequest\x1a-.controller.proto.v1.ListParticipantsResponse\"\x00\x12a\n" +
	"\n" +
	"ListRoutes\x12'.controlplane.proto.v1.RouteListRequest\x1a(.controlplane.proto.v1.RouteListResponse\"\x00BRZPgithub.com/agntcy/slim/control-plane/common/proto/controlplane/v1;controlplanev1b\x06proto3"

var (
	file_controlplane_v1_controlplane_proto_rawDescOnce sync.Once
	file_controlplane_v1_controlplane_proto_rawDescData []byte
)

func file_controlplane_v1_controlplane_proto_rawDescGZIP() []byte {
	file_controlplane_v1_controlplane_proto_rawDescOnce.Do(func() {
		file_controlplane_v1_controlplane_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_controlplane_v1_controlplane_proto_rawDesc), len(file_controlplane_v1_controlplane_proto_rawDesc)))
	})
	return file_controlplane_v1_controlplane_proto_rawDescData
}

var file_controlplane_v1_controlplane_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_controlplane_v1_controlplane_proto_msgTypes = make([]protoimpl.MessageInfo, 14)
var file_controlplane_v1_controlplane_proto_goTypes = []any{
	(NodeStatus)(0),                     // 0: controlplane.proto.v1.NodeStatus
	(RouteStatus)(0),                    // 1: controlplane.proto.v1.RouteStatus
	(*ConfigurationCommand)(nil),        // 2: controlplane.proto.v1.ConfigurationCommand
	(*Node)(nil),                        // 3: controlplane.proto.v1.Node
	(*NodeListRequest)(nil),             // 4: controlplane.proto.v1.NodeListRequest
	(*NodeEntry)(nil),                   // 5: controlplane.proto.v1.NodeEntry
	(*NodeListResponse)(nil),            // 6: controlplane.proto.v1.NodeListResponse
	(*AddRouteRequest)(nil),             // 7: controlplane.proto.v1.AddRouteRequest
	(*AddRouteResponse)(nil),            // 8: controlplane.proto.v1.AddRouteResponse
	(*DeleteRouteRequest)(nil),          // 9: controlplane.proto.v1.DeleteRouteRequest
	(*DeleteRouteResponse)(nil),         // 10: controlplane.proto.v1.DeleteRouteResponse
	(*CreateChannelRequest)(nil),        // 11: controlplane.proto.v1.CreateChannelRequest
	(*CreateChannelResponse)(nil),       // 12: controlplane.proto.v1.CreateChannelResponse
	(*RouteListRequest)(nil),            // 13: controlplane.proto.v1.RouteListRequest
	(*RouteEntry)(nil),                  // 14: controlplane.proto.v1.RouteEntry
	(*RouteListResponse)(nil),           // 15: controlplane.proto.v1.RouteListResponse
	(*v1.ConfigurationCommand)(nil),     // 16: controller.proto.v1.ConfigurationCommand
	(*v1.ConnectionDetails)(nil),        // 17: controller.proto.v1.ConnectionDetails
	(*v1.Connection)(nil),               // 18: controller.proto.v1.Connection
	(*v1.Subscription)(nil),             // 19: controller.proto.v1.Subscription
	(*v1.DeleteChannelRequest)(nil),     // 20: controller.proto.v1.DeleteChannelRequest
	(*v1.AddParticipantRequest)(nil),    // 21: controller.proto.v1.AddParticipantRequest
	(*v1.DeleteParticipantRequest)(nil), // 22: controller.proto.v1.DeleteParticipantRequest
	(*v1.ListChannelsRequest)(nil),      // 23: controller.proto.v1.ListChannelsRequest
	(*v1.ListParticipantsRequest)(nil),  // 24: controller.proto.v1.ListParticipantsRequest
	(*v1.SubscriptionListResponse)(nil), // 25: controller.proto.v1.SubscriptionListResponse
	(*v1.ConnectionListResponse)(nil),   // 26: controller.proto.v1.ConnectionListResponse
	(*v1.Ack)(nil),                      // 27: controller.proto.v1.Ack
	(*v1.ListChannelsResponse)(nil),     // 28: controller.proto.v1.ListChannelsResponse
	(*v1.ListParticipantsResponse)(nil), // 29: controller.proto.v1.ListParticipantsResponse
}
var file_controlplane_v1_controlplane_proto_depIdxs = []int32{
	16, // 0: controlplane.proto.v1.ConfigurationCommand.configuration_command:type_name -> controller.proto.v1.ConfigurationCommand
	17, // 1: controlplane.proto.v1.NodeEntry.connections:type_name -> controller.proto.v1.ConnectionDetails
	0,  // 2: controlplane.proto.v1.NodeEntry.status:type_name -> controlplane.proto.v1.NodeStatus
	5,  // 3: controlplane.proto.v1.NodeListResponse.entries:type_name -> controlplane.proto.v1.NodeEntry
	18, // 4: controlplane.proto.v1.AddRouteRequest.connection:type_name -> controller.proto.v1.Connection
	19, // 5: controlplane.proto.v1.AddRouteRequest.subscription:type_name -> controller.proto.v1.Subscription
	19, // 6: controlplane.proto.v1.DeleteRouteRequest.subscription:type_name -> controller.proto.v1.Subscription
	1,  // 7: controlplane.proto.v1.RouteEntry.status:type_name -> controlplane.proto.v1.RouteStatus
	14, // 8: controlplane.proto.v1.RouteListResponse.routes:type_name -> controlplane.proto.v1.RouteEntry
	7,  // 9: controlplane.proto.v1.ControlPlaneService.AddRoute:input_type -> controlplane.proto.v1.AddRouteRequest
	9,  // 10: controlplane.proto.v1.ControlPlaneService.DeleteRoute:input_type -> controlplane.proto.v1.DeleteRouteRequest
	3,  // 11: controlplane.proto.v1.ControlPlaneService.ListSubscriptions:input_type -> controlplane.proto.v1.Node
	3,  // 12: controlplane.proto.v1.ControlPlaneService.ListConnections:input_type -> controlplane.proto.v1.Node
	4,  // 13: controlplane.proto.v1.ControlPlaneService.ListNodes:input_type -> controlplane.proto.v1.NodeListRequest
	11, // 14: controlplane.proto.v1.ControlPlaneService.CreateChannel:input_type -> controlplane.proto.v1.CreateChannelRequest
	20, // 15: controlplane.proto.v1.ControlPlaneService.DeleteChannel:input_type -> controller.proto.v1.DeleteChannelRequest
	21, // 16: controlplane.proto.v1.ControlPlaneService.AddParticipant:input_type -> controller.proto.v1.AddParticipantRequest
	22, // 17: controlplane.proto.v1.ControlPlaneService.DeleteParticipant:input_type -> controller.proto.v1.DeleteParticipantRequest
	23, // 18: controlplane.proto.v1.ControlPlaneService.ListChannels:input_type -> controller.proto.v1.ListChannelsRequest
	24, // 19: controlplane.proto.v1.ControlPlaneService.ListParticipants:input_type -> controller.proto.v1.ListParticipantsRequest
	13, // 20: controlplane.proto.v1.ControlPlaneService.ListRoutes:input_type -> controlplane.proto.v1.RouteListRequest
	8,  // 21: controlplane.proto.v1.ControlPlaneService.AddRoute:output_type -> controlplane.proto.v1.AddRouteResponse
	10, // 22: controlplane.proto.v1.ControlPlaneService.DeleteRoute:output_type -> controlplane.proto.v1.DeleteRouteResponse
	25, // 23: controlplane.proto.v1.ControlPlaneService.ListSubscriptions:output_type -> controller.proto.v1.SubscriptionListResponse
	26, // 24: controlplane.proto.v1.ControlPlaneService.ListConnections:output_type -> controller.proto.v1.ConnectionListResponse
	6,  // 25: controlplane.proto.v1.ControlPlaneService.ListNodes:output_type -> controlplane.proto.v1.NodeListResponse
	12, // 26: controlplane.proto.v1.ControlPlaneService.CreateChannel:output_type -> controlplane.proto.v1.CreateChannelResponse
	27, // 27: controlplane.proto.v1.ControlPlaneService.DeleteChannel:output_type -> controller.proto.v1.Ack
	27, // 28: controlplane.proto.v1.ControlPlaneService.AddParticipant:output_type -> controller.proto.v1.Ack
	27, // 29: controlplane.proto.v1.ControlPlaneService.DeleteParticipant:output_type -> controller.proto.v1.Ack
	28, // 30: controlplane.proto.v1.ControlPlaneService.ListChannels:output_type -> controller.proto.v1.ListChannelsResponse
	29, // 31: controlplane.proto.v1.ControlPlaneService.ListParticipants:output_type -> controller.proto.v1.ListParticipantsResponse
	15, // 32: controlplane.proto.v1.ControlPlaneService.ListRoutes:output_type -> controlplane.proto.v1.RouteListResponse
	21, // [21:33] is the sub-list for method output_type
	9,  // [9:21] is the sub-list for method input_type
	9,  // [9:9] is the sub-list for extension type_name
	9,  // [9:9] is the sub-list for extension extendee
	0,  // [0:9] is the sub-list for field type_name
}

func init() { file_controlplane_v1_controlplane_proto_init() }
func file_controlplane_v1_controlplane_proto_init() {
	if File_controlplane_v1_controlplane_proto != nil {
		return
	}
	file_controlplane_v1_controlplane_proto_msgTypes[12].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_controlplane_v1_controlplane_proto_rawDesc), len(file_controlplane_v1_controlplane_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   14,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_controlplane_v1_controlplane_proto_goTypes,
		DependencyIndexes: file_controlplane_v1_controlplane_proto_depIdxs,
		EnumInfos:         file_controlplane_v1_controlplane_proto_enumTypes,
		MessageInfos:      file_controlplane_v1_controlplane_proto_msgTypes,
	}.Build()
	File_controlplane_v1_controlplane_proto = out.File
	file_controlplane_v1_controlplane_proto_goTypes = nil
	file_controlplane_v1_controlplane_proto_depIdxs = nil
}
