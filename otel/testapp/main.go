...
func main() {
	...
	channelNameStr := flag.String("channel-name", "", "Optional: channel name to receive telemetry, required if the exporter name is provided")
...
func handleSession(
	...
	for {
		...
		default:
			...
			if err != nil {
				errMsg := err.Error()
				if strings.Contains(errMsg, "session closed") {
					logger.Info("Session closed by peer", zap.Uint32("sessionId", sessionNum), zap.Error(err))
					return
				} else if strings.Contains(errMsg, "receive timeout waiting for message") {
