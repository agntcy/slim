...
```
task slim:wait-for-lb CONTROL_HOSTNAME=spire.admin.example
...
### 4. Create cluster federation resources on all three clusters
```
...
<details>
  ...
  This step creates the necessary Kubernetes resources to establish SPIRE federation between all three clusters in a full mesh configuration:
  
  - **FederationRelationship** resources defining trust relationships between all clusters
  - **ClusterFederatedTrustDomain** resources for cross-cluster trust across all domains
  - Network policies allowing federation traffic between all clusters
...
### 5. Deploy controller on admin cluster
```
task slim:contoller:deploy
```

<details>
  <summary>More Details on Centralized Controller</summary>
  ...
  The centralized Controller handles:
  ...
  - Centralized coordination and policy enforcement
</details>

### 6. Add controller endpoint to /etc/hosts
```
task slim:wait-for-lb CONTROL_HOSTNAME=slim-control.admin.example
```

### 7. Deploy SLIM on workload clusters (cluster-a & cluster-b)
```
task slim:deploy
```
...
# Check Controller on admin cluster
kubectl config use-context kind-admin.example
kubectl get deployment -n slim slim-control
kubectl get pods -n slim
...
# Controller logs
kubectl config use-context kind-admin.example
kubectl logs -n slim deployment/slim-control
```
...
<details>
  ...
  - Messages flow: Bob → SLIM(cluster-b) → Controller(admin) → SLIM(cluster-a) → Alice
...
```
# Delete SLIM from workload clusters
kubectl config use-context kind-cluster-a.example
task slim:delete
...
# Delete Controller from admin cluster
kubectl config use-context kind-admin.example
task slim:delete
