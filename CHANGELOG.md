...
#### Python Bindings: Removal of `receive` API

The legacy unified `receive` function (and `Slim.receive` method) has been **removed** to eliminate ambiguous semantics and accidental retention of session receive channels. It is replaced by an explicit two–stage pattern that separates:

1. Session discovery / global dequeue
2. Per–session message retrieval

New functions introduced:

- `get_any_message(PyService) -> (PySessionInfo, bytes)`: Returns the next publish message from any session (also serves as session discovery). Does NOT start per‑session buffering automatically.
- `listen_for_session(PyService, session_id: int)`: Registers a per‑session listener and begins buffering messages for that session.
- `get_message(PyService | Slim, session_id: int) -> bytes`: Retrieves (awaits) the next buffered message for the specified session. High‑level wrapper: `Slim.get_message(session_id)`.

High‑level `Slim` wrapper changes:

- `Slim.receive` removed.
- `Slim.create_session` now automatically calls `listen_for_session` for the newly created session.
- Session auto‑discovery now requires calling `get_any_message` directly (using the underlying service: `slim_bindings.get_any_message(slim.svc)`), then optionally invoking `listen_for_session` before per‑session `get_message` calls.

Rust service changes:

- Added `SessionContext.spawn_receiver` to detach the application receiver loop cleanly while retaining an `Arc<Session>` for lifecycle operations (e.g. deletion, configuration changes).
- Added accessor helpers: `session()`, `into_parts()`, `into_session()`.

Rationale:

- Avoids overloading a single API (`receive`) that previously had dual behavior (session discovery vs. per‑session reads) leading to subtle bugs and ordering issues.
- Clarifies ownership: Users explicitly opt into buffering for a session (`listen_for_session`) rather than accidentally holding the main channel open.
- Prepares for future optimization (e.g. selective backpressure, pluggable buffering strategies).

Migration Guide:

| Old Pattern | New Pattern |
| ----------- | ----------- |
| `session_info, msg = await slim.receive()` | `session_info, msg = await slim_bindings.get_any_message(slim.svc)` |
| `session_info, msg = await slim.receive(session=session_id)` | `await slim_bindings.listen_for_session(slim.svc, session_id)` then `msg = await slim.get_message(session_id)` |
| Auto start using first `receive()` result | Call `listen_for_session` explicitly after `get_any_message` |
| Background loop reading `receive()` for all sessions | Either: (a) loop on `get_any_message` + dispatch; or (b) per session tasks awaiting `get_message(session_id)` |

Example Before:
```python
session_info, _ = await slim.receive()
session_info, msg = await slim.receive(session=session_info.id)
```

Example After:
```python
session_info, _ = await slim_bindings.get_any_message(slim.svc)  # discover
await slim_bindings.listen_for_session(slim.svc, session_info.id)
msg = await slim.get_message(session_info.id)
```

Publishing & Reply (unchanged):
```python
await slim.publish(session_info, b"hello", remote_name)
reply = await slim.get_message(session_info.id)
```

Rust Session Loop Migration:
Before:
```rust
while let Some(Ok(msg)) = ctx.rx.recv().await { handle(msg); }
```
After:
```rust
let session = ctx.spawn_receiver(|mut rx, session, _meta| async move {
  while let Some(Ok(msg)) = rx.recv().await { handle(&session, msg); }
});
```

Removed Symbols:
- `slim_bindings.receive`
- `Slim.receive`

Action Required:
1. Replace all imports/usages of `receive`.
2. Add explicit `listen_for_session` for each session you want buffered (unless created via `Slim.create_session`, which already listens).
3. Use `get_any_message` only for discovery or truly unordered consumption across sessions.

If you need a compatibility shim, you can implement:
```python
async def compatibility_receive(slim_obj, session: int | None = None):
  if session is None:
    return await slim_bindings.get_any_message(slim_obj.svc)
  await slim_bindings.listen_for_session(slim_obj.svc, session)
  return slim_obj.sessions[session], await slim_obj.get_message(session)
```
but this is discouraged for new code.
