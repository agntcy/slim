...
func main() {
	...
	appNameStr := flag.String("app-name", "agntcy/otel/receiver-app", "Application name in the for org/ns/service")
	...
	// Parse app name from input string
	appName, err := common.SplitID(*appNameStr)
	if err != nil {
	...
	if err != nil {
		app.Destroy()
		log.Fatalf("Fail to connect to SLIM: %v", err)
	}
	defer app.Destroy() // Ensure cleanup on normal exit
	...
	if !*initiator {
		...
		initiateSessions(app, ctx, *mlsEnabled, connId)
	}
}

// initiateSessions creates and manages outgoing telemetry sessions
func initiateSessions(app *slim.BindingsAdapter, ctx context.Context, mlsEnabled bool, connId uint64) {
	...
	metricsChannel := slim.Name{
		Components: []string{"agntcy", "otel", "telemetr-metrics"},
		Id:         nil,
	}
	...
	exporterName := slim.Name{
		Components: []string{"agntcy", "otel", "exporter"},
		Id:         nil,
	}
...
func handleSession(app *slim.BindingsAdapter, session *slim.FfiSessionContext, sessionNum int, ctx context.Context) {
	defer func() {
