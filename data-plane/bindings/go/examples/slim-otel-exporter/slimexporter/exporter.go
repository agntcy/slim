...
var (
	...
	// Shared session listener state
	sharedExporters     = make(map[SignalType]*slimExporter)
	sharedExportersMu   sync.RWMutex
...
func (e *slimExporter) start(ctx context.Context, host component.Host) error {
	...
	if len(toInvite) == 0 {
		...
		// Register this exporter and start shared listener if not already started
		sharedExportersMu.Lock()
		sharedExporters[e.signalType] = e
		sharedExportersMu.Unlock()
...
func listenForAllSessions(app *slim.BindingsAdapter, logger *zap.Logger) {
	...
	for {
		...
		var targetSignal SignalType
		if len(channelComponent) >= 7 && channelComponent[len(channelComponent)-7:] == "metrics" {
			targetSignal = SignalMetrics
		} else if len(channelComponent) >= 6 && channelComponent[len(channelComponent)-6:] == "traces" {
			targetSignal = SignalTraces
		} else if len(channelComponent) >= 4 && channelComponent[len(channelComponent)-4:] == "logs" {
		...
		// Find and assign session to the appropriate exporter
		sharedExportersMu.RLock()
		exporter, exists := sharedExporters[targetSignal]
		sharedExportersMu.RUnlock()

		if !exists {
			logger.Warn("No exporter registered for signal type",
				zap.String("signal", string(targetSignal)),
				zap.String("session_name", channelComponent))
			continue
		}

		// Assign session to the appropriate channel
		switch targetSignal {
		case SignalMetrics:
			exporter.metricsChannel = session
		case SignalTraces:
			exporter.tracesChannel = session
		case SignalLogs:
			exporter.logsChannel = session
		}

		logger.Info("Assigned incoming session to channel",
			zap.String("signal", string(targetSignal)),
			zap.String("session_name", channelComponent))

		// Check if all registered exporters have their sessions assigned
		sharedExportersMu.RLock()
		allAssigned := true
		for signalType, exp := range sharedExporters {
			switch signalType {
			case SignalMetrics:
				if exp.metricsChannel == nil {
					allAssigned = false
				}
			case SignalTraces:
				if exp.tracesChannel == nil {
					allAssigned = false
				}
			case SignalLogs:
				if exp.logsChannel == nil {
					allAssigned = false
				}
			}
			if !allAssigned {
...
func (e *slimExporter) shutdown(ctx context.Context) error {
	...
	// Unregister this exporter from shared state
	sharedExportersMu.Lock()
	delete(sharedExporters, e.signalType)
	sharedExportersMu.Unlock()
...
func (e *slimExporter) pushTraces(ctx context.Context, td ptrace.Traces) error {
	...
	if e.tracesChannel == nil {
		e.logger.Info("trace channel not set yet, drop the message")
		return nil
	}
...
func (e *slimExporter) pushMetrics(ctx context.Context, md pmetric.Metrics) error {
	...
	if e.metricsChannel == nil {
		e.logger.Info("metric channel not set yet, drop the message")
		return nil
	}
...
func (e *slimExporter) pushLogs(ctx context.Context, ld plog.Logs) error {
	...
	if e.logsChannel == nil {
		e.logger.Info("log channel not set yet, drop the message")
		return nil
	}
