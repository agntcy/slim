...
async def test_multi_session_single_client_request_reply(server):
    ...
    await responder.connect({"endpoint": "http://127.0.0.1:12390", "tls": {"insecure": True}})
    
    # Track sessions and responses
    sessions_handled = {}
    total_responses = 0
    
    async def central_responder_task():
        ...
        async with responder:
            ...
            while total_responses < expected_total:
                try:
                    ...
                    session_active = True
                    while session_active and total_responses < expected_total:
                        try:
                            ...
                            if msg:
                                ...
                                print(f"[Central-Responder] Session {session_id} message {sessions_handled[session_id]['messages']}: {decoded}")
                                
                                # Send response back using publish_to
                                response = f"response-{total_responses}".encode()
                    ...
                    print(f"[Central-Responder] General timeout - {total_responses}/{expected_total} responses")
                    break
            
            print(f"[Central-Responder] Finished - handled {len(sessions_handled)} sessions, {total_responses} responses")
    
    async def create_publisher(pub_id):
        ...
        await publisher.connect({"endpoint": "http://127.0.0.1:12390", "tls": {"insecure": True}})
    ...
    responder_task = asyncio.create_task(central_responder_task())
    ...
    for i in range(publishers_count):
        ...
        task = asyncio.create_task(create_publisher(i))
        publisher_tasks.append(task)
        await asyncio.sleep(1.5)
    
    # Wait for all tasks to complete
    await asyncio.gather(responder_task, *publisher_tasks)
    
    # Verify results
    print(f"\n=== Request-Reply Multi-Session Results ===")
    ...
    # Assertions
    assert len(sessions_handled) >= 1, f"Expected at least 1 session, got {len(sessions_handled)}"
    assert total_responses >= 1, f"Expected at least 1 response, got {total_responses}"
    
    print(f"  Request-reply multi-session test passed: {len(sessions_handled)} sessions, {total_responses} responses")
...
async def test_topic_subscription_pubsub_pattern(server):
    ...
    # Shared topic for communication
    ...
    await coordinator.connect({"endpoint": "http://127.0.0.1:12391", "tls": {"insecure": True}})
    ...
    async def coordinator_task():
        ...
        async with coordinator:
            print(f"[Coordinator] Ready to coordinate {participants_count} participants on topic")
            
            expected_total = participants_count * messages_per_participant
            ...
            while len(completed_participants) < participants_count:
                try:
                    ...
                    if session_id not in participant_sessions:
                        ...
                        participant_sessions[session_id] = {"participant_id": participant_id, "messages": 0}
                        print(f"[Coordinator] New participant session {session_id} from Participant-{participant_id}")
                    
                    # Handle messages from this participant
                    session_active = True
                    while session_active and len(completed_participants) < participants_count:
                        try:
                            recv_session, msg = await asyncio.wait_for(
                            ...
                            if msg:
                                ...
                                if participant_sessions[session_id]["messages"] >= messages_per_participant:
                    ...
                    print(f"[Coordinator] General timeout - {len(completed_participants)}/{participants_count} completed")
                    break
            
            print(f"[Coordinator] Finished coordinating - {messages_received} messages handled")
    ...
    async def create_participant(participant_id):
        ...
        await participant.connect({"endpoint": "http://127.0.0.1:12391", "tls": {"insecure": True}})
        ...
        async with participant:
            ...
            for msg_num in range(messages_per_participant):
                ...
                print(f"[Participant-{participant_id}] Publishing to topic: {payload.decode()}")
    ...
    # Start coordinator
    coord_task = asyncio.create_task(coordinator_task())
    await asyncio.sleep(2)
    ...
    for i in range(participants_count):
        ...
        task = asyncio.create_task(create_participant(i))
        participant_tasks.append(task)
        await asyncio.sleep(2)
    
    # Wait for all tasks
    await asyncio.gather(coord_task, *participant_tasks)
    
    # Verify results
    print(f"\n=== Pubsub Multi-Session Results ===")
    ...
    for session_id, stats in participant_sessions.items():
        ...
        print(f"Participant-{participant_id} (Session {session_id}): {msg_count} messages")
    
    # Assertions
    assert len(participant_sessions) >= 1, f"Expected at least 1 participant session, got {len(participant_sessions)}"
    assert messages_received >= 1, f"Expected at least 1 message, got {messages_received}"
    
    print(f"Pubsub multi-session test passed: {len(participant_sessions)} sessions, {messages_received} messages")
...
async def test_multiple_clients_single_handler(server):
    ...
    await handler.connect({"endpoint": "http://127.0.0.1:12392", "tls": {"insecure": True}})
    
    received_sessions = {}
    client_message_counts = {}
    completed_clients = set()
    
    async def single_handler_task():
            ...
            while len(completed_clients) < client_count:
                try:
                    ...
                    if session_id not in received_sessions:
                        ...
                        received_sessions[session_id] = {"client_id": client_id, "messages": 0}
                        client_message_counts[client_id] = 0
                        print(f"[Single-Handler] New session from Client-{client_id}: {session_id}")
                    
                    # Handle messages from this client
                    session_active = True
                    while session_active and len(completed_clients) < client_count:
                        try:
                            recv_session, msg = await asyncio.wait_for(
                    ...
                    print(f"[Single-Handler] General timeout - {len(completed_clients)}/{client_count} completed")
                    break
            
            print(f"[Single-Handler] Finished handling {len(received_sessions)} client sessions")
    
    async def create_client_publisher(client_id):
        ...
        await client.connect({"endpoint": "http://127.0.0.1:12392", "tls": {"insecure": True}})
    ...
    handler_task = asyncio.create_task(single_handler_task())
    await asyncio.sleep(2)
    
    # Start multiple independent clients with staggered timing
    client_tasks = []
    for i in range(client_count):
        task = asyncio.create_task(create_client_publisher(i))
    ...
    print(f"\n=== Multiple Clients → Single Handler Results ===")
    ...
    # Assertions
    assert len(received_sessions) >= 1, f"Expected at least 1 client session, got {len(received_sessions)}"
    assert total_messages >= 1, f"Expected at least 1 message, got {total_messages}"
    
    print(f"  Multiple clients → single handler test passed: {len(received_sessions)} sessions, {total_messages} messages")
...
async def test_bidirectional_communication_streaming(server):
    ...
    coordinator = await create_slim(coordinator_name, "secret")
    await coordinator.connect({"endpoint": "http://127.0.0.1:12393", "tls": {"insecure": True}})
    ...
    async def bidirectional_coordinator_task():
        """Coordinator handling bidirectional communication"""
        nonlocal messages_from_clients, responses_sent
        
        async with coordinator:
            print(f"[Bidirectional-Coordinator] Ready for {client_count} bidirectional clients")
            ...
            while messages_from_clients < expected_messages:
                try:
                    ...
                    if session_id not in bidirectional_sessions:
                        ...
                        bidirectional_sessions[session_id] = {"client_id": client_id, "messages": 0}
                        print(f"[Bidirectional-Coordinator] New bidirectional session {session_id} from Client-{client_id}")
                    
                    # Handle bidirectional messages from this session
                    session_active = True
                    while session_active and messages_from_clients < expected_messages:
                        try:
                            recv_session, msg = await asyncio.wait_for(
                                coordinator.receive(session=session_id), timeout=20
                            )
                            ...
                            if msg:
                                ...
                                response = f"bidirectional-response-{messages_from_clients}".encode()
                                await coordinator.publish(recv_session, response, topic)
                                responses_sent += 1
                                
                                # Check if client completed
                                if bidirectional_sessions[session_id]["messages"] >= messages_per_client:
                    ...
                    print(f"[Bidirectional-Coordinator] General timeout - {messages_from_clients}/{expected_messages}")
                    break
            
            print(f"[Bidirectional-Coordinator] Finished - {messages_from_clients} messages, {responses_sent} responses")
    
    async def create_bidirectional_client(client_id):
        """Create client for bidirectional communication"""
        client_name = slim_bindings.PyName(org, ns, f"bidirectional-client-{client_id}")
        client = await create_slim(client_name, "secret")
        await client.connect({"endpoint": "http://127.0.0.1:12393", "tls": {"insecure": True}})
        ...
        print(f"[Bidirectional-Client-{client_id}] Created bidirectional session {session_info.id}")
        ...
        async with client:
            ...
            for msg_num in range(messages_per_client):
                ...
                try:
                    ...
                    if response:
                        print(f"[Bidirectional-Client-{client_id}] Got response: {response.decode()}")
                except asyncio.TimeoutError:
                    print(f"[Bidirectional-Client-{client_id}] No response for message {msg_num}")
    ...
    coord_task = asyncio.create_task(bidirectional_coordinator_task())
    await asyncio.sleep(2)
    ...
    for i in range(client_count):
        ...
        task = asyncio.create_task(create_bidirectional_client(i))
        client_tasks.append(task)
        await asyncio.sleep(2)
    
    # Wait for all tasks
    await asyncio.gather(coord_task, *client_tasks)
    
    # Verify results
    print(f"\n=== Bidirectional Communication Results ===")
    ...
    # Assertions
    assert len(bidirectional_sessions) >= 1, f"Expected at least 1 bidirectional session, got {len(bidirectional_sessions)}"
    assert messages_from_clients >= 1, f"Expected at least 1 message from clients, got {messages_from_clients}"
    assert responses_sent >= 1, f"Expected at least 1 response sent, got {responses_sent}"
    
    print(f"  Bidirectional communication test passed: {len(bidirectional_sessions)} sessions, {messages_from_clients} messages, {responses_sent} responses")
...
async def test_with_mls_encryption_secure_sessions(server):
    ...
    await topic_subscriber.connect({"endpoint": "http://127.0.0.1:12394", "tls": {"insecure": True}})
    ...
    async def topic_subscriber_handler():
        """Subscriber handling multiple MLS sessions on the same topic"""
        async with topic_subscriber:
            print(f"[Topic-Subscriber] Subscribed to MLS topic, expecting {different_clients_count} clients")
            
            total_expected = different_clients_count * messages_per_client
            messages_received = 0
            ...
            while len(completed_topic_clients) < different_clients_count:
                try:
                    ...
                    session_info, payload = await asyncio.wait_for(
                        topic_subscriber.receive(), timeout=60  # MLS requires more time
                    )
                    ...
                    if session_id not in topic_sessions:
                        ...
                        print(f"[Topic-Subscriber] New MLS session from Client-{client_id} on topic: {session_id}")
                    ...
                    session_active = True
                    while session_active and len(completed_topic_clients) < different_clients_count:
                        try:
                            ...
                            if encrypted_msg:
                                ...
                                print(f"[Topic-Subscriber] MLS decrypted from Client-{client_id}: {decrypted}")
                    ...
                    print(f"[Topic-Subscriber] MLS topic timeout - {len(completed_topic_clients)}/{different_clients_count} clients")
                    break
            
            print(f"[Topic-Subscriber] Finished handling {len(topic_sessions)} MLS sessions on topic")
            print(f"[Topic-Subscriber] Total encrypted messages received: {messages_received}")
    
    async def create_topic_client(client_id):
        ...
        await topic_client.connect({"endpoint": "http://127.0.0.1:12394", "tls": {"insecure": True}})
        ...
        async with topic_client:
            for msg_num in range(messages_per_client):
                ...
                print(f"[Topic-Client-{client_id}] Publishing encrypted to topic: {payload.decode()}")
                ...
                try:
                    ...
                    if encrypted_response:
                        ...
                        print(f"[Topic-Client-{client_id}] Got topic response: {encrypted_response.decode()}")
                    
                except asyncio.TimeoutError:
                    print(f"[Topic-Client-{client_id}] No topic response for message {msg_num}")
    ...
    subscriber_task = asyncio.create_task(topic_subscriber_handler())
    ...
    for i in range(different_clients_count):
        ...
        task = asyncio.create_task(create_topic_client(i))
        client_tasks.append(task)
    ...
    print(f"\n=== MLS Topic Multi-Session Results ===")
    ...
    for session_id, stats in client_stats.items():
        client_id = stats["client_id"]
        msg_count = stats["messages"]
        print(f"Topic-Client-{client_id} (Session {session_id}): {msg_count} encrypted messages")
    
    # Assertions
    assert len(topic_sessions) >= 2, f"Expected at least 2 MLS topic sessions, got {len(topic_sessions)}"
    assert len(completed_topic_clients) >= 2, f"Expected at least 2 completed topic clients, got {len(completed_topic_clients)}"
    
    print(f"  MLS Topic Multi-Session test passed: {len(topic_sessions)} sessions, {len(completed_topic_clients)} clients")
...
async def test_without_mls_encryption_plain_sessions(server):
    ...
    topic_subscriber = await create_slim(topic_subscriber_name, "secret")
    await topic_subscriber.connect({"endpoint": "http://127.0.0.1:12395", "tls": {"insecure": True}})
    ...
    async def plain_topic_subscriber_handler():
        """Subscriber handling multiple plain sessions on the same topic"""
        async with topic_subscriber:
            print(f"[Plain-Topic-Subscriber] Subscribed to plain topic, expecting {different_clients_count} clients")
            ...
            while len(completed_topic_clients) < different_clients_count:
                try:
                    # Receive session invitations or messages on the topic
                    session_info, payload = await asyncio.wait_for(
                        topic_subscriber.receive(), timeout=30  # Plain text is faster
                    )
                    ...
                    if session_id not in topic_sessions:
                        client_id = len(topic_sessions)
                        print(f"[Plain-Topic-Subscriber] New plain session from Client-{client_id} on topic: {session_id}")
                        topic_sessions[session_id] = session_info
                        client_stats[session_id] = {"client_id": client_id, "messages": 0}
                    ...
                    while session_active and len(completed_topic_clients) < different_clients_count:
                        try:
                            ...
                            if plain_msg:
                                ...
                                print(f"[Plain-Topic-Subscriber] Plain text from Client-{client_id}: {decoded}")
                                ...
                                # Check if this client completed all messages
                                if client_stats[session_id]["messages"] >= messages_per_client:
                                    print(f"[Plain-Topic-Subscriber] Client-{client_id} completed on topic")
                                    completed_topic_clients.add(client_id)
                                    session_active = False
                    ...
                    print(f"[Plain-Topic-Subscriber] Plain topic timeout - {len(completed_topic_clients)}/{different_clients_count} clients")
                    break
            
            print(f"[Plain-Topic-Subscriber] Finished handling {len(topic_sessions)} plain sessions on topic")
            print(f"[Plain-Topic-Subscriber] Total plain messages received: {messages_received}")
    
    async def create_plain_topic_client(client_id):
        """Create a client that connects to the plain topic"""
        client_name = slim_bindings.PyName(org, ns, f"plain-topic-client-{client_id}")
        topic_client = await create_slim(client_name, "secret")
        await topic_client.connect({"endpoint": "http://127.0.0.1:12395", "tls": {"insecure": True}})
        ...
        print(f"[Plain-Topic-Client-{client_id}] Created plain session {session_info.id} for topic")
        ...
        async with topic_client:
            for msg_num in range(messages_per_client):
                ...
                print(f"[Plain-Topic-Client-{client_id}] Publishing plain to topic: {payload.decode()}")
                ...
                try:
                    ...
                    if plain_response:
                        ...
                        print(f"[Plain-Topic-Client-{client_id}] Got topic response: {plain_response.decode()}")
                    
                except asyncio.TimeoutError:
                    print(f"[Plain-Topic-Client-{client_id}] No topic response for message {msg_num}")
    ...
    subscriber_task = asyncio.create_task(plain_topic_subscriber_handler())
    await asyncio.sleep(2)
    ...
    for i in range(different_clients_count):
        ...
        task = asyncio.create_task(create_plain_topic_client(i))
        client_tasks.append(task)
    ...
    print(f"\n=== Plain Topic Multi-Session Results ===")
    ...
    for session_id, stats in client_stats.items():
        client_id = stats["client_id"]
        msg_count = stats["messages"]
        print(f"Plain-Topic-Client-{client_id} (Session {session_id}): {msg_count} plain messages")
    
    # Assertions
    assert len(topic_sessions) >= 2, f"Expected at least 2 plain topic sessions, got {len(topic_sessions)}"
    assert len(completed_topic_clients) >= 2, f"Expected at least 2 completed plain topic clients, got {len(completed_topic_clients)}"
    
    print(f" Plain Topic Multi-Session test passed: {len(topic_sessions)} sessions, {len(completed_topic_clients)} clients")
