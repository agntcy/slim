...
async fn main() {
    ...
    let svc = config
        ...
        .get_mut(&svc_id)
        .expect("service slim/0 not found");

    // create local app
    let app_name = Name::from_strings(["agntcy", "default", "publisher"]).with_id(id);

    let provider = SharedSecret::new("a", slim_testing::utils::TEST_VALID_SECRET).expect("provider");
    let verifier = SharedSecret::new("a", slim_testing::utils::TEST_VALID_SECRET).expect("verifier");
    let (app, _rx) = svc
    ...
    let destination = routes.first().expect("at least one route").clone();
    let session_config =
        SlimSessionConfig::default().with_session_type(ProtoSessionType::PointToPoint);
    let res = app
    ...
    // get the session
    let (session_ctx, _completion) = res.unwrap();
                        ...
                        Some(message) => {
                            ...
                            let msg = message.unwrap();
                                    ...
                                    if let Some(slim_datapath::api::ProtoPublishType(publish)) = msg.message_type.as_ref() {
                                        let app_payload = publish
                                            .get_payload()
                                            .as_application_payload()
                                            .expect("application payload");
                                        let payload = &app_payload.blob;
