...
mod tests {
    ...
    async fn test_create_session() {
        ...
        // Session is created but, with no peer or SLIM processing,
        // the completion handle is expected to resolve with an error
        let (_session, completion_handle) = app
            .create_session(config.clone(), dst.clone(), 0, None)
            .await
            .unwrap();

        // The completion future should complete within a reasonable time
        let result = tokio::time::timeout(std::time::Duration::from_secs(1), completion_handle)
            .await
            .expect("Session creation future should not hang indefinitely");

        // And it should fail because there is no peer / closed SLIM side
        assert!(
            result.is_err(),
            "Session creation should have failed due to missing peer or closed channel"
        );
    }
    ...
    async fn test_handle_message_from_app() {
        ...
        // the first message should now be a subscribe used to
        // program the route towards the remote endpoint
        let first_msg = rx_slim
            .recv()
            .await
            .expect("no message received")
            .expect("error");
        ...
        // after the routing subscribe, a discovery request should be
        // generated by the session just created; read it from SLIM
