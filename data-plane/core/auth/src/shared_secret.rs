...
// MLS signature key generation. On native targets we defer to the AWS-LC MLS
// crypto provider so the bytes are valid for the negotiated ciphersuite. On
// wasm32 that provider is unavailable (and WebCrypto is async-only), so we emit
// random placeholder bytes; the MLS layer always regenerates and installs the
// ciphersuite-correct P-256 keys via `set_signature_keys` during init.
#[cfg(not(target_arch = "wasm32"))]
