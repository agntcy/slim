...
async fn test_spiffe_provider_initialization() {
    ...
    'test_block: {
        ...
        // Simulate the MLS layer setting signature keys (keys are now always
        // generated by the MLS crypto provider and pushed into the identity provider).
        provider
            .set_signature_keys(vec![1u8; 32], vec![2u8; 32])
            .expect("set_signature_keys should succeed");
...
async fn test_spiffe_grpc_jwt_auth() {
    ...
    'test_block: {
        ...
        let channel = match client_cfg.to_channel().await {
            ...
            Ok(_) => {
                tracing::error!("expected grpc channel");
                should_panic = true;
                break 'test_block;
            }
            Err(e) => {
