...
mod tests {
    ...
    async fn test_messaging() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let charlie_group_id = charlie.process_welcome(&charlie_add_res.welcome_message).await?;
        ...
        charlie.process_commit(&daniel_add_res.commit_message).await?;

        let daniel_group_id = daniel.process_welcome(&daniel_add_res.welcome_message).await?;
        ...
        let commit = alice.remove_member(&charlie_add_res.member_identity).await?;
