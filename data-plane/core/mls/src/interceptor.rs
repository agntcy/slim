                ...
                if let Some(message_type) = &mut msg.message_type {
                    if let MessageType::Publish(publish) = message_type {
                        if let Some(content) = &mut publish.msg {
                            ...
                            if was_encrypted {
                                msg.metadata.insert("mls_encrypted".to_string(), "true".to_string());
                            }
                        }
                    }
                }
...
mod tests {
    ...
    async fn test_mls_interceptor_without_group() {
        let identity_provider = Arc::new(
            FileBasedIdentityProvider::new("/tmp/test_mls_interceptor_no_group").unwrap()
        );
        ...
        msg.metadata.insert("mls_group_id".to_string(), "test_group".to_string());
    ...
    async fn test_mls_interceptor_with_group() {
        let alice_provider = Arc::new(
            FileBasedIdentityProvider::new("/tmp/test_mls_interceptor_alice").unwrap()
        );
        let bob_provider = Arc::new(
            FileBasedIdentityProvider::new("/tmp/test_mls_interceptor_bob").unwrap()
        );
        ...
        alice_msg.metadata.insert("mls_group_id".to_string(), group_id_str);
        
        alice_interceptor.on_msg_from_app(&mut alice_msg);
        
        assert_ne!(alice_msg.get_payload().unwrap().blob, original_payload);
        assert_eq!(alice_msg.metadata.get("mls_encrypted").map(|v| v.as_str()), Some("true"));
