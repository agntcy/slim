...
const GROUP_FILE_TEMPLATE: &str = r#"//go:build slim_multicast
...
pub fn generate(request: CodeGeneratorRequest) -> Result<CodeGeneratorResponse> {
    ...
    for file_name in &request.file_to_generate {
        ...
        let mut group_service_definitions = String::new();
        let mut group_extra_imports: HashMap<String, String> = HashMap::new();

        // Seed group_extra_imports with types_import if provided.
        if let (Some(path), Some(alias)) = (&types_import_path, &types_alias) {
            let last_component = path.split('/').next_back().unwrap_or("");
            let line = if alias.as_str() == last_component {
                format!("\"{}\"", path)
            ...
            group_extra_imports.insert(alias.clone(), line);
        }
        ...
        for service in &file_descriptor.service {
            ...
            for method in &service.method {
                ...
                // Collect imports for group file too
                if let Some((alias, line)) = resolve_proto_type(
                ...
                .1
                {
                    group_extra_imports.insert(alias, line);
                }
                if let Some((alias, line)) = resolve_proto_type(
            ...
            // Generate group client interface
            ...
            group_service_definitions.push_str(&group_client_interface);
        }
        ...
        if services_found {
            ...
            // Generate group file
            let group_file_name = file_name
                .strip_suffix(".proto")
                .unwrap_or(file_name)
                .to_string()
                + "_slimrpc_group.pb.go";

            let mut group_import_lines: Vec<&String> = group_extra_imports.values().collect();
            group_import_lines.sort();
            let group_proto_imports: String = group_import_lines
                .into_iter()
                .map(|s| format!("\t{}\n", s))
                .collect();

            let group_content = GROUP_FILE_TEMPLATE
                .replace("{{PROTO_FILE}}", file_name)
                .replace("{{PACKAGE}}", &go_package)
                .replace("{{PROTO_IMPORTS}}", &group_proto_imports)
                .replace("{{GROUP_SERVICE_DEFINITIONS}}", &group_service_definitions);

            response.file.push(File {
                name: Some(group_file_name),
                insertion_point: None,
                content: Some(group_content),
                generated_code_info: None,
            });
        }
    }
...
mod tests {
    ...
    fn test_generate_group_client() {
        ...
        // Two files generated: regular and group
        assert_eq!(response.file.len(), 2);

        // First file is the regular one
        let regular_name = response.file[0].name.as_ref().unwrap();
        assert_eq!(regular_name, "example_slimrpc.pb.go");

        // Second file is the group file
        let group_name = response.file[1].name.as_ref().unwrap();
        assert_eq!(group_name, "example_slimrpc_group.pb.go");

        let group_content = response.file[1].content.as_ref().unwrap();

        // Build tag present
        assert!(group_content.contains("//go:build slim_multicast"));

        // TestGroupClient interface
        assert!(group_content.contains("TestGroupClient interface"));

        // Constructor function
        assert!(group_content.contains("NewTestGroupClient"));

        // All four channel methods referenced
        assert!(group_content.contains("CallMulticastUnaryAsync"));
        assert!(group_content.contains("CallMulticastUnaryStreamAsync"));
        assert!(group_content.contains("CallMulticastStreamUnary"));
        assert!(group_content.contains("CallMulticastStreamStream"));
    }
}
